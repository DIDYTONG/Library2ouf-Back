...
const addInProcess = async (req, res) => {
    try {
        ...
        book.status = "fini";
        library.bookStatus.inProccess.push(book);
        ...
        res
            .status(200)
            .json({ message: "Book attribué avec succès à l'utilisateur (In process)" });
    } catch (error) {
        console.error(error);
        res
            .status(500)
