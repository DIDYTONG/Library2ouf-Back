...
const addReadLibrary = async (req, res) => {
    try {
        ...
        book.status = "fini";
        library.bookStatus.read.push(book);
        ...
        res
            .status(200)
            .json({ message: "Book attribué avec succès à l'utilisateur (Read)" });
    } catch (error) {
        console.error(error);
        res
            .status(500)
